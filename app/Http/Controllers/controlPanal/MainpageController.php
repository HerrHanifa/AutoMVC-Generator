...
class MainpageController extends Controller
{
    ...
    public function delete($id)
    {
        ...
        return redirect()->route('mainpage.index.web')
    ...
    public function store(Request $request)
    {
        ...
        foreach ($files as $key => $file) {
            ...
            if ($file->isValid() && in_array($file->extension(), ['jpeg', 'png', 'jpg', 'gif', 'svg','webp','mp4'])) {
                ...
                $newItem[$key] = ImageHelper::handleImageUpload($file, 'uploads/Mainpage_images');
            }
        }
        
        // إنشاء العنصر الجديد في قاعدة البيانات
        Mainpage::create($newItem);
    ...
    {
        // العثور على العنصر باستخدام الـ ID
        $updateItem = Mainpage::findOrFail($id);
        ...
        foreach ($files as $key => $file) {
            ...
            if ($file->isValid() && in_array($file->extension(), ['jpeg', 'png', 'jpg', 'gif', 'svg', 'webp'])) {
                ...
                $updateData[$key] = ImageHelper::handleImageUpload($file, 'uploads/Mainpage_images');
            }
        }
        ...
        // إعادة التوجيه إلى صفحة العرض
        return redirect()->route('mainpage.index.web')->with('success', 'Item updated successfully!');
    }
